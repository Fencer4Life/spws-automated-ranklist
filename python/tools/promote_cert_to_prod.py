...
def sync_results(dry_run: bool) -> tuple[int, int]:
    ...
    for r in cert_results:
        ...
        if key not in prod_results:
            if dry_run:
                print(f"    DRY: INSERT result {r[1]} {r[2]} in {r[0]} (#{r[3]})", file=sys.stderr)
            else:
                ...
                try:
                    ...
                    print(f"    WARN: insert failed {r[1]} {r[2]} in {r[0]}: {e}", file=sys.stderr)
