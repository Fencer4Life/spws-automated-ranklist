...
class TestConnectorForwardsUrl:
    def test_find_or_create_forwards_p_url_results(self):
        ...
        db.find_or_create_tournament(
            7, "EPEE", "M", "V2", "2026-04-01", "PPW",
            url_results="https://ftl/v2")
...
def _match(id_fencer, place, name):
    return StageMatchResult(scraped_name=name, place=place, id_fencer=id_fencer,
...
def _commit_with(source_kind, source_url):
    ...
    parsed = ParsedTournament(
        source_kind=source_kind, results=[], parsed_date=date(2026, 4, 1),
        weapon="EPEE", gender="M", category_hint="V2", season_end_year=2026,
        source_url=source_url)
    pctx = PipelineContext(parsed=parsed, overrides=Overrides(),
...
@pytest.mark.parametrize("kind", [
    SourceKind.FENCINGTIME_XML, SourceKind.EVF_API, SourceKind.FILE_IMPORT,
])
    ...
    def test_parsed_source_url_and_kind(self):
        ...
        class _Client:
            ...
            def get(self, url):
                ...
                if "/results/data/" in url:
                    return _Resp(js=[{"id": "f1", "name": "KOWALSKI Jan",
                                      "place": "1", "country": "POL"}])
        ...
        with patch("python.scrapers.ftl_auth.get_authed_ftl_client", return_value=_Client()), \
            ...
            ingest_cli.ingest_event_from_url(
                ...
                db=db, notifier=TelegramNotifier(None, None))
...
class TestStagingMentionsUrl:
    def _run(self, tmp_path, live_rows, source_decisions):
        ...
        rep = [{"section": "COMMIT"}]  # minimal; renderers read live rows + decisions
        ctx = Context()
        ...
        return [p for p in tmp_path.glob("*.md")
                if not p.name.endswith(".diff.md")][0].read_text()
    ...
    def test_committed_shows_persisted_url_and_summary(self, tmp_path):
        ...
        live = [
            {"id_tournament": 1, "txt_code": "PPW3-V2-M-EPEE-2025-2026",
             "enum_age_category": "V2", "enum_weapon": "EPEE", "enum_gender": "M",
             "dt_tournament": "2025-12-13", "url_results": "https://r/v2",
             "int_participant_count": 19},
            {"id_tournament": 2, "txt_code": "PPW3-V3-M-EPEE-2025-2026",
             "enum_age_category": "V3", "enum_weapon": "EPEE", "enum_gender": "M",
             "dt_tournament": "2025-12-13", "url_results": "https://r/v3-preserved",
             "int_participant_count": 8},
        ]
        # only V2 was (re)written by this run's keep-rule; V3's url is pre-existing
        decisions = [
            {"name": "kat. 2", "url": "https://r/v2", "weapon": "EPEE", "gender": "M",
