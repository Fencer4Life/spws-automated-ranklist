...
class TestIngestCli:
    ...
    def test_from_storage_does_not_auto_delete(self):
        ...
        with (
            patch("python.pipeline.ingest_cli.create_db_connector") as mock_create_db,
            patch("python.pipeline.ingest_cli.process_xml_file") as mock_process,
        ):
            mock_db = MagicMock()
            ...
            mock_db.assert_not_called()  # db connector object never invoked directly
    ...
    def test_main_catches_exception_and_notifies(self):
        ...
        with (
            ...
            patch(
                "sys.argv",
                ["ingest_cli", str(FIXTURES / "single_category.xml"), "--season-end-year", "2026"],
            ),
        ):
            mock_notifier = MagicMock()
...
class TestDraftManagementCli:
    ...
    def test_commit_draft_calls_rpc_and_prints(self, capsys):
        ...
        with (
            ...
            patch("python.pipeline.ingest_cli.TelegramNotifier") as mock_notifier_cls,
            patch(
                "sys.argv", ["ingest_cli", "--commit-draft", "11111111-1111-1111-1111-111111111111"]
            ),
        ):
    ...
    def test_commit_draft_zero_counts_triggers_warning(self):
        ...
        with (
            ...
            patch("python.pipeline.ingest_cli.TelegramNotifier") as mock_notifier_cls,
            patch(
                "sys.argv", ["ingest_cli", "--commit-draft", "00000000-0000-0000-0000-000000000000"]
            ),
        ):
            mock_store = MagicMock()
            mock_store.commit.return_value = {
    ...
    def test_discard_draft_calls_rpc(self):
        ...
        with (
            ...
            patch(
                "sys.argv",
                ["ingest_cli", "--discard-draft", "22222222-2222-2222-2222-222222222222"],
            ),
        ):
            mock_store = MagicMock()
    ...
    def test_resume_run_id_reads_drafts_and_prints_diff(self, capsys):
        ...
        with (
            ...
            patch(
                "sys.argv",
                ["ingest_cli", "--resume-run-id", "33333333-3333-3333-3333-333333333333"],
            ),
        ):
            mock_store = MagicMock()
                ...
                [
                    {
                        ...
                        "dt_tournament": "2026-04-01",
                        "url_results": "https://test/x",
                    }
                ],
                [{"txt_code": "TEST-V0", "int_place": 1, "enum_match_method": "AUTO_MATCHED"}],
            )
            mock_store_cls.return_value = mock_store
            mock_notifier_cls.return_value = MagicMock()

            main()
            mock_store.read_drafts.assert_called_once_with("33333333-3333-3333-3333-333333333333")
            captured = capsys.readouterr()
    ...
    def test_commit_and_discard_mutually_exclusive(self):
        ...
        with (
            ...
            patch("sys.argv", ["ingest_cli", "--commit-draft", "aaa", "--discard-draft", "bbb"]),
        ):
