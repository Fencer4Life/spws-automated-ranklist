...
def _make_parsed(results):
    ...
    return ParsedTournament(
        source_kind=SourceKind.CERT_REF, results=results,
        parsed_date=date(2026, 4, 1), weapon="EPEE", gender="M",
        organizer_hint="SPWS", category_hint="V1", raw_pool_size=len(results),
...
def _fencer(id_, surname, first, by):
    return {"id_fencer": id_, "txt_surname": surname, "txt_first_name": first,
            ...
            "txt_nationality": "POL", "enum_gender": "M", "json_name_aliases": []}
...
class TestChainThroughRunFlow:
    def test_below_min_drop_escalates_on_loss_telegram(self, monkeypatch):
        ...
        svc = Services(db=_db([_fencer(101, "KOWALSKI", "Jan", 1980)]),
                       config={"parsed": parsed, "overrides": Overrides(),
    ...
    def test_clean_ingest_chains_post_commit_without_escalation(self, monkeypatch):
        ...
        parsed = _make_parsed([
            ParsedResult(source_row_id="t:1", fencer_name="KOWALSKI Jan", place=1, fencer_country="POL"),
            ParsedResult(source_row_id="t:2", fencer_name="NOWAK Adam", place=2, fencer_country="POL"),
        ])
        svc = Services(db=_db([_fencer(101, "KOWALSKI", "Jan", 1980),
