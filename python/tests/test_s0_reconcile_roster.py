...
class FakeDB:
    ...
    def update_fencer_birth_year(
        ...
        self.updated.append(
            {"id_fencer": fencer_id, "int_birth_year": birth_year,
             "estimated": estimated}
        )
...
def _fencer(
    id_fencer, surname, first, *, by=None, estimated=False,
    nationality="PL", gender=None, aliases=None,
):
    return {
        "id_fencer": id_fencer,
        "txt_surname": surname,
...
def _ctx(results, *, event_code="PPW3-2025-2026", category_hint=None,
         gender=None, season_end_year=2026, source=SourceKind.FTL):
...
class TestCreateNewFencer:
    def test_unknown_foreign_woman_created_not_glued(self):
        ...
        db = FakeDB([_fencer(1, "BARAN", "Anna", by=1970, nationality="PL",
                             gender="F")])
        # Combined women's pool, unmarked → V-cat unknown.
        ctx = _ctx([_result("RABAB Fatima", country="MAR", marker=None)],
                   category_hint=None, gender="F")
    ...
    def test_created_with_marker_gets_midpoint(self):
        ...
        db = FakeDB([])
        ctx = _ctx([_result("NOWAK Jan", country="POL", marker="2")],
                   category_hint=None, gender="M")
        stages.s0_reconcile_roster(ctx, db)
        created = db.inserted[0]
    ...
    def test_single_cat_bracket_uses_hint_midpoint(self):
        ...
        db = FakeDB([])
        ctx = _ctx([_result("KOWALSKA Ewa", country="POL")],
                   category_hint="V3", gender="F")
        stages.s0_reconcile_roster(ctx, db)
    ...
    def test_alias_match_not_recreated(self):
        ...
        db = FakeDB([_fencer(1, "SPLAWA-NEYMAN", "Piotr",
                            aliases=["NEYMAN Piotr"], nationality="PL")])
        ctx = _ctx([_result("NEYMAN Piotr", country="POL", marker="2")],
                   category_hint=None)
        stages.s0_reconcile_roster(ctx, db)
        assert db.inserted == []
    ...
    def test_same_name_different_nationality_created(self):
        ...
        ctx = _ctx([_result("KOWAL Jan", country="UKR", marker="2")],
                   category_hint=None)
        stages.s0_reconcile_roster(ctx, db)
        assert len(db.inserted) == 1
    ...
    def test_polish_2letter_vs_3letter_no_duplicate(self):
        ...
        db = FakeDB([_fencer(1, "WISNIEWSKI", "Adam", by=1970,
                            nationality="PL")])
        ctx = _ctx([_result("WISNIEWSKI Adam", country="POL", marker="2")],
                   category_hint=None)
        stages.s0_reconcile_roster(ctx, db)
        assert db.inserted == []
...
class TestReconcile:
    def test_estimated_conflict_reestimated_keep_flag(self):
        ...
        db = FakeDB([_fencer(1, "DABROWSKI", "Marek", by=1991,
                            estimated=True, nationality="PL")])
        ctx = _ctx([_result("DABROWSKI Marek", country="POL")],
                   category_hint="V2")
        stages.s0_reconcile_roster(ctx, db)
    ...
    def test_confirmed_conflict_downgraded_and_flagged(self):
        ...
        db = FakeDB([_fencer(1, "ZIELINSKI", "Tomasz", by=1991,
                            estimated=False, nationality="PL")])
        ctx = _ctx([_result("ZIELINSKI Tomasz", country="POL")],
                   category_hint="V3")
        stages.s0_reconcile_roster(ctx, db)
    ...
    def test_no_conflict_no_write(self):
        ...
        db = FakeDB([_fencer(1, "LIS", "Jan", by=1971, estimated=False,
                            nationality="PL")])
    ...
    def test_unknown_vcat_no_reconcile(self):
        ...
        db = FakeDB([_fencer(1, "MAJ", "Ola", by=1991, estimated=True,
                            nationality="PL")])
        ctx = _ctx([_result("MAJ Ola", country="POL", marker=None)],
                   category_hint=None)
        stages.s0_reconcile_roster(ctx, db)
        assert db.updated == []
...
class TestInternationalSkip:
    @pytest.mark.parametrize("code", ["PEW63e-2025-2026", "MEW-2025-2026",
                                      "MSW-2025-2026"])
    def test_international_event_skipped(self, code):
        """10.5.1 — PEW/MEW/MSW events: no creation, no reconciliation."""
        db = FakeDB([_fencer(1, "SMITH", "John", by=1991, estimated=False,
                            nationality="GBR")])
        ctx = _ctx([_result("BRAND New", country="GBR"),
                    _result("SMITH John", country="GBR")],
                   event_code=code, category_hint="V2")
        stages.s0_reconcile_roster(ctx, db)
        assert db.inserted == []
        assert db.updated == []
        assert ctx.created_fencers == []
    ...
    def test_domestic_event_runs(self):
        ...
        db = FakeDB([])
        ctx = _ctx([_result("NEW Person", country="POL")],
                   event_code="PPW3-2025-2026", category_hint="V2")
        stages.s0_reconcile_roster(ctx, db)
        assert len(db.inserted) == 1
...
class TestIdempotenceAndConflicts:
    def test_rerun_no_duplicate_create(self):
        ...
        assert len(db.inserted) == 1
        ctx2 = _ctx([_result("UNIQUE Person", country="POL", marker="2")],
                    category_hint=None)
    ...
    def test_reconcile_idempotent(self):
        ...
        db = FakeDB([_fencer(1, "REPEAT", "Guy", by=1991, estimated=True,
                            nationality="PL")])
    ...
    def test_cross_bracket_conflict_flagged_not_thrashed(self):
        ...
        db = FakeDB([_fencer(1, "TWICE", "Bob", by=1991, estimated=True,
                            nationality="PL")])
...
class TestExcludedRows:
    def test_excluded_row_skipped(self):
        ...
        db = FakeDB([])
        ctx = _ctx([_result("DROP Me", country="POL", marker="2",
                            excluded=True)],
                   category_hint=None)
        stages.s0_reconcile_roster(ctx, db)
        assert db.inserted == []
...
class TestStagingMdSection:
    ...
    def test_created_and_downgrade_blocks_render(self):
        ...
        ctx = self._ctx_with(
            created_fencers=[{
                "id_fencer": 9, "scraped_name": "RABAB Fatima",
                "nationality": "MAR", "vcat": None, "birth_year": None,
                "estimated": False, "source": "FTL",
            }],
            reconciled_fencers=[{
                "id_fencer": 5, "scraped_name": "ZIELINSKI Tomasz",
                "vcat": "V3", "old_birth_year": 1991, "new_birth_year": 1961,
                "was_confirmed": True, "source": "FTL",
            }],
        )
