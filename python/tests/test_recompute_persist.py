...
def _m(id_fencer, place, by, weapon="EPEE", gender="M"):
    return StageMatchResult(
        scraped_name=str(id_fencer), place=place, id_fencer=id_fencer,
        confidence=100.0, method="AUTO_MATCHED", governed_birth_year=by,
        weapon=weapon, gender=gender, tournament_date=date(2026, 2, 21))
...
def _ctx(matches, *, id_event=7, season=2026, event_code="PPW4-2025-2026"):
    ...
    ctx = Context()
    pctx = PipelineContext(parsed=None, overrides=Overrides(),
                           season_end_year=season, event_code=event_code)
...
class TestRecomputePersist:
    ...
    def test_relocates_to_new_vcat_bracket(self):
        ...
        matches = [_m(1, 2, 1970, weapon="EPEE"),   # age 56 -> V2
                   _m(2, 1, 1960, weapon="EPEE")]   # age 66 -> V3 (relocated)
        db = _db()
        _run(_ctx(matches), db)
        vcats = {c.args[3]: c.kwargs.get("participant_count")
        ...
        counts = {c.args[0] if False else None: None}  # placeholder
        ing = {c.args[0]: c.kwargs["participant_count"]
    ...
    def test_clears_emptied_bracket(self):
        ...
        db = _db(existing_tournaments=[
            {"id_tournament": 99, "enum_weapon": "EPEE", "enum_gender": "M",
    ...
    def test_rewritten_bracket_not_cleared(self):
        ...
        db = _db(existing_tournaments=[
            {"id_tournament": 88, "enum_weapon": "EPEE", "enum_gender": "M",
    ...
    def test_row_shape_and_status(self):
        ...
        assert rows[0] == {
            "id_fencer": 5, "int_place": 1, "txt_scraped_name": "5",
