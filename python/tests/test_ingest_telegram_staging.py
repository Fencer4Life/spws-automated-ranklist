...
def _event_db():
    ...
    db.find_event_by_code.return_value = {
        "id_event": 4, "txt_code": "PPW4-2025-2026",
    ...
    def test_override_persisted_and_used(self):
        ...
        with ps[0], ps[1], ps[2], ps[3]:
            ingest_cli.ingest_event_from_url(
                ...
                db=db, notifier=_silent_notifier(), url_event_override=new_url)
...
class TestStagingToTelegram:
    def test_send_helper_posts_full_and_diff(self):
        ...
        ingest_cli._send_staging_via_telegram(
            notifier, "PPW4-2025-2026", post, n_tournaments=24)
...
class TestStagingFormatterMdTarget:
    def test_md_target_storage_routes_write(self, tmp_path):
        ...
        with patch("python.pipeline.plugins.staging_formatter.write_for_event") as wfe, \
             patch("python.pipeline.plugins.staging_formatter._fetch_event_meta",
...
class TestCliFlags:
    def test_flags_thread_into_from_url(self):
        from python.pipeline import ingest_cli
        argv = ["prog", "--flow", "ingest_domestic", "--event-code", "PPW4-2025-2026",
                "--season-end-year", "2026", "--from-url",
                "--url-event", "https://ftl/NEW", "--send-telegram", "--md-target", "storage"]
