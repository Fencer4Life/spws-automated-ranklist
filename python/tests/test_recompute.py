...
def _db(event_results, by_map=None):
    ...
    db.fetch_birth_years_batch.return_value = (
        by_map or {r["id_fencer"]: r["int_birth_year"] for r in event_results})
    db.ingest_results.return_value = {"ok": True}
    return db


def _recompute(db, id_event=7, season=2026):
    cfg = {"id_event": id_event, "season_end_year": season,
...
class TestLoadCommitted:
    def test_builds_matches_from_stored_rows(self):
        ...
        rows = [{"id_fencer": 1, "place": 1, "int_birth_year": 1980},
                {"id_fencer": 2, "place": 2, "int_birth_year": 1960}]
        ctx = Context()
        p = LoadCommitted()
        ctx._begin(p)
        p.run(ctx, Services(db=_db(rows), config={
            "id_event": 7, "season_end_year": 2026,
...
class TestRecomputeFlow:
    def test_repartitions_by_governed_by(self):
        ...
        rows = [{"id_fencer": 1, "place": 1, "int_birth_year": 1980},   # 46 -> V1
                {"id_fencer": 2, "place": 2, "int_birth_year": 1960}]   # 66 -> V3
    ...
    def test_recompute_twice_equals_once(self):
        ...
        norm = lambda fv: {k: sorted(m.id_fencer for m in v) for k, v in fv.items()}
...
class TestSourceRetention:
    def test_parse_source_retains_artifact_path(self):
        ...
        parsed = ParsedTournament(
            ...
            organizer_hint="SPWS", category_hint="V1", season_end_year=2026)
        ctx = Context()
        p = ParseSource()
        ctx._begin(p)
        p.run(ctx, Services(config={
            "parsed": parsed, "source": "retained",
            "source_artifact_path": "/retained/ppw4.xml",
            "season_end_year": 2026, "event_code": "PPW4-2025-2026"}))
        ctx._end()
