-- =============================================================================
-- Master Fencer List — tbl_fencer
-- 378 SPWS members; birth year only; club and nationality not tracked.
-- Auto-loaded via config.toml sql_paths glob after seed.sql.
-- Note: birth year alone is sufficient for SPWS age-category rules (calendar-year-based).
-- NULL int_birth_year = year unknown.
-- =============================================================================
INSERT INTO tbl_fencer (txt_surname, txt_first_name, int_birth_year) VALUES
    ('ADAMCZEWSKI',             'Wojciech',      1952),
    ('ADAMCZYK',                'Grzegorz',      NULL),
    ('ADAMSKA',                 'Maria',         1979),
    ('ANDERSCH',                'Robert',        NULL),
    ('ATANASSOW',               'Aleksander',    1969),
    ('AUGUSTOWSKI',             'Waldemar',      1959),
    ('AUGUSTYN',                'Kajetan',       NULL),
    ('BARAN',                   'Agata',         1986),
    ('BARAŃSKI',                'Wacław',        1964),
    ('BARTUSIK',                'Grzegorz',      1990),
    ('BAZAK',                   'Jacek',         1974),
    ('BEDNARSKA',               'Halina',        1968),
    ('BEDNARZ',                 'Przemysław',    1984),
    ('BETLEJ',                  'Daniel',        1977),
    ('BETLEJA',                 'Artur',         1984),
    ('BOBUSIA',                 'Dariusz',       1984),
    ('BOBUSIA',                 'Jarosław',      1985),
    ('BONIK-NINARD',            'Agata',         NULL),
    ('BORKOWSKA',               'Halina',        1956),
    ('BORKOWSKI',               'Andrzej',       1950),
    ('BORYSIUK',                'Zbigniew',      1953),
    ('BROMBEREK',               'Angelika',      1988),
    ('BROSCH',                  'Artur',         NULL),
    ('BUJKO',                   'Jacek',         NULL),
    ('BUJKO',                   'Paulina',       NULL),
    ('BURLIKOWSKI',             'Bartosz',       1972),
    ('CANIARD',                 'Henry',         NULL),
    ('CHMIELEWSKA',             'Emilia',        1986),
    ('CHOJNACKI',               'Tomasz',        1987),
    ('CHOJNECKI',               'Mateusz',       1988),
    ('CHUDYCKI',                'Artur',         1960),
    ('CIEPŁY',                  'Tomasz',        1980),
    ('CYGAŃSKI',                'Tomasz',        1973),
    ('CZAJKOWSKI',              'Marcin',        1979),
    ('ĆWIORO',                  'Krzysztof',     1960),
    ('ĆWIORO',                  'Tomasz',        1985),
    ('DAMIAN',                  'Mateusz',       1988),
    ('DARUL',                   'Tomasz',        1978),
    ('DĄBROWSKA-MANDRELA',      'Diana',         1982),
    ('DOBRZAŃSKI',              'Maciej',        1978),
    ('DOMAŃSKI',                'Sławomir',      NULL),
    ('DRAPELLA',                'Maciej',        1964),
    ('DRAPELLA',                'Magdalena',     1996),
    ('DROBCZYK',                'Paweł',         1980),
    ('DROBIŃSKI',               'Leszek',        1967),
    ('DUDEK',                   'Mariusz',       1973),
    ('DWORAKOWSKA-BANAŚ',       'Agnieszka',     NULL),
    ('DYNAREK',                 'Aleksander',    1990),
    ('DZIUBIŃSKI',              'Mateusz',       1988),
    ('FORAJTER',                'Roman',         1977),
    ('FRAŚ',                    'Feliks',        NULL),
    ('FRYDRYCH',                'Aleksander',    NULL),
    ('FRYDRYCH',                'Szymon',        1986),
    ('FRYDRYCKI',               'Bartłomiej',    NULL),
    ('FRYDRYCKI',               'Mariusz',       1980),
    ('FUHRMANN',                'Urlike',        1963),
    ('FURMANIAK',               'Andrzej',       1947),
    ('GAJDA',                   'Krzysztof',     1993),
    ('GAJDA',                   'Leszek',        1965),
    ('GAJDA',                   'Zbigniew',      1967),
    ('GANSZCZYK',               'Anna',          1972),
    ('GANSZCZYK',               'Marcin',        1974),
    ('GAWLE',                   'Katarzyna',     NULL),
    ('GĄSIOROWSKI',             'Maciej',        1970),
    ('GĘZIKIEWICZ',             'Marcin',        1991),
    ('GIERS-ROMEK',             'Monika',        1984),
    ('GINZERY',                 'Tomas',         NULL),
    ('GOLA',                    'Maciej',        1981),
    ('GRABOWSKI',               'Alan',          1991),
    ('GRABOWSKI',               'Romuald',       1962),
    ('GRABOWSKI',               'Sebastian',     1988),
    ('GRACZYK',                 'Anna',          1985),
    ('GRACZYK',                 'Bogdan',        1984),
    ('GRACZYKOWSKI',            'Mateusz',       NULL),
    ('GRĘZIKIEWICZ',            'Marcin',        1991),
    ('GRIPAS',                  'Artiom',        1984),
    ('GRODNER',                 'Michał',        1960),
    ('GROMADA',                 'Roland',        1985),
    ('GRZEGOREK',               'Norbert',       1970),
    ('GRZYWACZ',                'Mirosław',      1972),
    ('GUENTHER',                'Grzegorz',      NULL),
    ('GUZY',                    'Adrian',        1983),
    ('GWIAZDA',                 'Paweł',         1970),
    ('HAJDAS',                  'Martyna',       NULL),
    ('HAŁOŃ',                   'Bartłomiej',    1984),
    ('HAŚKO',                   'Sergiusz',      1974),
    ('HEŁKA',                   'Jacek',         1968),
    ('HERONIMEK',               'Leszek',        1959),
    ('IRZYK',                   'Sabina',        NULL),
    ('JABŁOŃSKA',               'Ewa',           1971),
    ('JADCZUK',                 'Wojciech',      1980),
    ('JANKOWSKI',               'Kamil',         1992),
    ('JAROSZEK',                'Zbigniew',      1972),
    ('JASIELCZUK',              'Igor',          1987),
    ('JASZCZAK',                'Piotr',         1973),
    ('JENDRYŚ',                 'Marek',         1974),
    ('JEROZOLIMSKI',            'Marek',         1990), -- ESTIMATED from V0 category (PPW4 Excel)
    ('JUSZKIEWICZ',             'Piotr',         1954),
    ('KACZMAREK',               'Paweł',         1970),
    ('KAMIŃSKA',                'Gabriela',      1979),
    ('KASPRZYK-KUŹNIAK',        'Michalina',     1977),
    ('KASZTELOWICZ',            'Piotr',         NULL),
    ('KAZIK',                   'Martin',        NULL),
    ('KĘDZIORA',                'Agata',         1988),
    ('KIEROŃSKI',               'Tomasz',        NULL),
    ('KIERSZNICKI',             'Ryszard',       1955),
    ('KLIMECKA',                'Dorota',        1977),
    ('KŁOS',                    'Iwona',         NULL),
    ('KMIECIK',                 'Adam',          1986),
    ('KOBIERSKI',               'Krzysztof',     1976),
    ('KOCÓR',                   'Agata',         1990),
    ('KOLER',                   'Jarosław',      1989),
    ('KOLLAR',                  'Gabriel',       NULL),
    ('KOŁUCKI',                 'Michał',        NULL),
    ('KOŃCZYŃSKI',              'Adam',          1984),
    ('KORNAŚ',                  'Jarosław',      1985),
    ('KORONA',                  'Przemysław',    1976),
    ('KORYGA',                  'Bartłomiej',    1992),
    ('KORZH',                   'Valery',        NULL),
    ('KOSIŃSKI',                'Łukasz',        1983),
    ('KOSTRZEWA',               'Ireneusz',      1965),
    ('KOSZYK',                  'Agnieszka',     NULL),
    ('KOTERSKI',                'Paweł',         1971),
    ('KOTTS',                   'Radosław',      1970),
    ('KOWALCZYK',               'Piotr',         NULL),
    ('KOWALEWSKI',              'Rafał',         1979),
    ('KOWALSKA',                'Milena',        1984),
    ('KOWALSKI',                'Bartosz',       1995),
    ('KOWALSKI',                'Tomasz',        1975),
    ('KOZAK',                   'Marta',         1990),
    ('KOZIEJOWSKI',             'Sebastian',     NULL),
    ('KRAMARZ',                 'Konrad',        1986),
    ('KRAWCZYK',                'Paweł',         1954),
    ('KRAWCZYK',                'Paweł',         1989),
    ('KROCHMALSKI',             'Jakub',         1976),
    ('KRZEMIŃSKI',              'Mariusz',       1962),
    ('KUCIĘBA',                 'Piotr',         1979),
    ('KULKA',                   'Dawid',         1979),
    ('KULKA',                   'Sławomir',      NULL),
    ('LASKUS',                  'Krystyna',      1969),
    ('LELONEK',                 'Tomasz',        NULL),
    ('LIPKOWSKA',               'Dominika',      1984),
    ('ŁĘCKI',                   'Krzysztof',     1991),
    ('MAINKA',                  'Andrzej',       1949),
    ('MALINOWSKI',              'Piotr',         NULL),
    ('MAŁASIŃSKI',              'Adam',          1958),
    ('MAŁASIŃSKI',              'Maciej',        NULL),
    ('MANDRELA',                'Jarosław',      1976),
    ('MARASEK',                 'Tomasz',        1976),
    ('MAZIK',                   'Aleksander',    1970),
    ('MENCWAL',                 'Adam',          NULL),
    ('MIECZYŃSKI',              'Adam',          NULL),
    ('MIKULICKI',               'Arkadiusz',     NULL),
    ('MILCZAREK',               'Renata',        1968),
    ('MILOVA',                  'Tatiana',       1965),
    ('MŁYNEK',                  'Janusz',        1984),
    ('MŁYNEK',                  'Janusz',        1951),
    ('MORDEL',                  'Adam',          1985),
    ('MROCZYK',                 'Ireneusz',      1957),
    ('MULSON',                  'Irena',         1956),
    ('MUTWIL',                  'Marcin',        1970),
    ('NIECZKOWSKI',             'Tomasz',        1977),
    ('NIKALAICHUK',             'Aliaksandr',    1963),
    ('NOWAK',                   'Marta',         1979),
    ('NOWAK',                   'Szymon',        NULL),
    ('NOWAKOWSKI',              'Andrzej',       1952),
    ('NOWICKI',                 'Robert',        1970),
    ('NOWICKI',                 'Wiesław',       1964),
    ('OLSZEWSKI',               'Mikołaj',       1968),
    ('OSSOWSKI',                'Wojciech',      1961),
    ('OWCZAREK',                'Elżbieta',      1957),
    ('OWCZAREK',                'Ewelina',       1987),
    ('OWCZAREK',                'Hubert',        1976),
    ('PAKUŁA',                  'Łukasz',        1978),
    ('PANZ',                    'Marian',        NULL),
    ('PARDUS',                  'Borys',         1970),
    ('PAWŁOWSKI',               'Łukasz',        1980), -- ESTIMATED from V1 category (PPW4 Excel)
    ('PELIŃSKA',                'Beata',         1979),
    ('PIGUŁA',                  'Dariusz',       1977),
    ('PILARSKA',                'Barbara',       1974),
    ('PILUTKIEWICZ',            'Igor',          1975),
    ('PLUCIŃSKI',               'Paweł',         1969),
    ('POJMAŃSKA',               'Katarzyna',     NULL),
    ('POKRYWA',                 'Bartosz',       1984),
    ('POKRZYWA',                'Mariusz',       1959),
    ('POPRAWA',                 'Mariusz',       1960),
    ('POŚPIESZNY',              'Sławomir',      1969),
    ('PRĘGOWSKI',               'Jerzy',         1947),
    ('PRZYSTAJKO',              'Daniel',        1985),
    ('PRZYSUCHA',               'Paweł',         NULL),
    ('PYZIK',                   'Zdzisław',      1960),
    ('RAJKIEWICZ',              'Radosław',      1982),
    ('REBONOK',                 'Andrzej',       NULL),
    ('REMIAN',                  'Paulina',       NULL),
    ('RUDY',                    'Andrzej',       1973),
    ('RUDZIŃSKI',               'Piotr',         1975),
    ('RUT',                     'Agnieszka',     NULL),
    ('RUTECKI',                 'Bogdan',        NULL),
    ('RUTECKI',                 'Paweł',         1978),
    ('RZESZUTKO',               'Jakub',         1984),
    ('SADOWIŃSKA',              'Adrianna',      1970),
    ('SADOWSKA',                'Małgorzata',    1982),
    ('SAJEWICZ',                'Izabela',       1987),
    ('SAMECKA-NACZYŃSKA',       'Martyna',       1985),
    ('SAMSONOWICZ',             'Maciej',        1978),
    ('SERAFIN',                 'Marek',         1967),
    ('SERWATKA',                'Marek',         1968),
    ('SĘKOWSKI',                'Maciej',        1981),
    ('SIDOR',                   'Marek',         NULL),
    ('SKOCZEK',                 'Artur',         NULL),
    ('SOBIERAJ',                'Wojciech',      1955),
    ('SOKOL',                   'Vratislav',     NULL),
    ('SOSNOWSKA',               'Aniela',        1952),
    ('STANIEWICZ',              'Witold',        1967),
    ('STAŃCZYK',                'Agnieszka',     1971),
    ('STOCKI',                  'Piotr',         NULL),
    ('STOLARSKI',               'Krzysztof',     NULL),
    ('STOŁOWSKI',               'Mariusz',       1963),
    ('SUROWIEC',                'Tomasz',        1962),
    ('SZCZERBOWSKI',            'Lech',          1972),
    ('SZCZĘSNY',                'Jacek',         1956),
    ('SZEPIETOWSKI',            'Rafał',         NULL),
    ('SZERSZEŃSKA',             'Aleksandra',    NULL),
    ('SZKATULSKA',              'Katarzyna',     NULL),
    ('SZMIDT',                  'Grzegorz',      1969),
    ('SZTUBA',                  'Mariusz',       1968),
    ('SZURLEJ',                 'Agata',         1975),
    ('SZYMAŃSKI',               'Adam',          1976),
    ('SZYMKOWIAK',              'Krzysztof',     NULL),
    ('TARANCZEWSKI',            'Janusz',        1949),
    ('TECŁAW',                  'Ewelina',       NULL),
    ('TECŁAW',                  'Robert',        1977),
    ('KOŃCZYŁO',                'Tomasz',        1973),
    ('TOMASZEWSKA',             'Hanna',         1974),
    ('TOMCZAK',                 'Ireneusz',      1968),
    ('TRACZ',                   'Jerzy',         1966),
    ('TRISCORNIA',              'Anton',         1990),
    ('TRYBUS',                  'Wiktor',        NULL),
    ('TRZENSIOK',               'Bernard',       1970),
    ('TUSZYŃSKI',               'Piotr',         1986),
    ('WALAS',                   'Zuzanna',       NULL),
    ('WALCZEWSKI',              'Konrad',        NULL),
    ('WALECKA',                 'Wanda',         1973),
    ('WALENCIUK',               'Urszula',       NULL),
    ('WALESIAK',                'Stanisław',     1991),
    ('WASILCZUK',               'Beata',         1971),
    ('WASIOŁKA',                'Sebastian',     1976),
    ('WĘCŁAWSKI',               'Mirosław',      1970),
    ('WHITLEY',                 'Gary',          1961),
    ('WIERZBICKI',              'Jacek',         1970),
    ('WINGROWICZ',              'Mariusz',       1973),
    ('WIŚNIEWSKI',              'Bernard',       1982),
    ('WITOSŁAWSKI',             'Przemysław',    NULL),
    ('WITOSŁAWSKI',             'Tomasz',        1988),
    ('WITOWSKI',                'Maciej',        NULL),
    ('WITTENBECK',              'Maciej',        NULL),
    ('WOJCIECHOWSKI',           'Marek',         1960),
    ('WOJTAS',                  'Bogdan',        1969),
    ('WRONA',                   'Grzegorz',      1965),
    ('WROŃSKI',                 'Stanisław',     1950),
    ('WYLĘGAŁA',                'Jerzy',         1949),
    ('ZABŁOCKI',                'Michał',        1964),
    ('ZAJĄC',                   'Michał',        1982),
    ('ZAWALICH',                'Leszek',        1969),
    ('ZAWROTNIAK',              'Przemysław',    1974),
    ('ZIELIŃSKI',               'Dariusz',       1969),
    ('ZIEMECKI',                'Grzegorz',      NULL),
    ('ZIMNY',                   'Lech',          1955),
    ('ŻUKOWSKI',                'Wojciech',      1969),
    ('ŻYCZKOWSKI',              'Adam',          1960),
    ('ADAMS',                   'Richard',       NULL),
    ('ALCSER',                  'Norbert',       NULL),
    ('ANNA-LISE',               'Mion',          NULL),
    ('ARONOWITSCH',             'Niklas',        NULL),
    ('BOCHEŃSKI',               'JACEK',         NULL),
    ('BOROWIEC',                'Maciej',        NULL),
    ('CASSERLY',                'Colm',          NULL),
    ('CHARKIEWICZ',             'Paweł',         NULL),
    ('CHIAROMONTE',             'Francesco',     NULL),
    ('CHUDY',                   'Tomasz',        1980), -- ESTIMATED from V1 category (PPW4 Excel)
    ('DONKE',                   'Ryszard',       NULL),
    ('DROBINSKI',               'Leszek',        NULL),
    ('EJCHSZTET',               'Mariusz',       NULL),
    ('FARAGO',                  'József',        NULL),
    ('FORNAL',                  'Mateusz',       NULL),
    ('FRAŚ',                    'Felix',         NULL),
    ('FUHRMANN',                'Ulrike',        NULL),
    ('GAWLE',                   'Katarzyna (kat 1)', NULL),
    ('GERTSMAN',                'Alex',          NULL),
    ('GIBULA',                  'Marcin',        NULL),
    ('GOLD',                    'Oleg',          NULL),
    ('GRZYB',                   'Bianka',        NULL),
    ('GÓRNA',                   'Karolina',      NULL),
    ('IWAŃCZUK',                'Tomasz',        NULL),
    ('JASIŃSKI',                'Tomasz',        NULL),
    ('KARMAN',                  'Irene',         NULL),
    ('KARWAT',                  'Aleksandra',    NULL),
    ('KLAMAN',                  'Mateusz',       NULL),
    ('KLEPACKI',                'Denis',         NULL),
    ('KONARSKI',                'Marcin',        NULL),
    ('KORONA',                  'Radosław',      NULL),
    ('KORONA-TRZEBSKI',         'Przemysław',    NULL),
    ('KOTSEV',                  'Ivan',          NULL),
    ('KRUJALSKIENE',            'Julija',        NULL),
    ('KRUJALSKIS',              'Gotfridas',     NULL),
    ('KRUJASKIS',               'Gotfridas',     NULL),
    ('KURBATSKYI',              'Stepan',        NULL),
    ('KUZMICHOVA',              'Svitlana',      NULL),
    ('KŁUSEK',                  'Damian',        NULL),
    ('LEAHEY',                  'John',          NULL),
    ('LEVY',                    'EMMANUEL',      NULL),
    ('LISOWSKI',                'Igor',          1990), -- ESTIMATED from V0 category (PPW4 Excel)
    ('LISOWSKI',                'Robert',        NULL),
    ('LYNCH',                   'Pat',           NULL),
    ('MACKO',                   'Marcin',        NULL),
    ('MADDEN',                  'Gerard',        NULL),
    ('MARNIAK',                 'Ksenia',        1980), -- ESTIMATED from V1 category (PPW4 Excel)
    ('MAZIK',                   'Alksander',     NULL),
    ('MCGINNITY',               'Marie',         NULL),
    ('MCQUEEN',                 'Andy',          NULL),
    ('METZA',                   'Oskar',         NULL),
    ('MIKOŁAJCZUK',             'Norbert',       NULL),
    ('MIKULICKA',               'Joanna',        NULL),
    ('MOCEK',                   'Sławomir',      NULL),
    ('MURRAY',                  'Claire',        NULL),
    ('NIKALAICHUK',             'Aleksander',    NULL),
    ('ODOLAK',                  'Jarosław',      NULL),
    ('OLBRYCHSKI',              'Antoni',        NULL),
    ('ORIFICI',                 'Adelchi',       NULL),
    ('OWCZAREK',                'Elzbieta',      NULL),
    ('PARELL',                  'Mikołaj',       1990), -- ESTIMATED from V0 category (PPW4 Excel)
    ('PLEWIŃSKI',               'Piotr',         NULL),
    ('POPOVIĆ',                 'Milos',         NULL),
    ('PRAHA-TSAREHRADSKA',      'Nadiia',        NULL),
    ('PRZYBYŁOWSKI',            'Michał',        NULL),
    ('PURGINA',                 'Marian',        NULL),
    ('PĘCZEK',                  'Sandra',        NULL),
    ('QUEVRAIN',                'Michel',        NULL),
    ('REDZIŃSKI',               'Michał',        NULL),
    ('ROMANOWICZ',              'Aleksiej',      NULL),
    ('RUSEK',                   'Roman',         NULL),
    ('RZEPECKA',                'Martyna',       1990), -- ESTIMATED from V0 category (PPW4 Excel)
    ('SADOWIŃSKA',              'Adriana',       NULL),
    ('SCHOLZ',                  'Jurgen',        NULL),
    ('SERAFIN',                 'Błażej',        NULL),
    ('SKRYPKA',                 'Glib',          NULL),
    ('SOBCZAK',                 'Sławomir',      NULL),
    ('SOMERS',                  'Jan',           NULL),
    ('SPIRINA',                 'Ekaterina',     NULL),
    ('SPŁAWA-NEYMAN',           '(0) MACIEJ',    NULL),
    ('SPŁAWA-NEYMAN',           'MACIEJ',        NULL),
    ('STANISŁAWSKI',            'Albert',        NULL),
    ('STAŃCZYK',                'Marcin',        1980), -- ESTIMATED from V1 category (PPW4 Excel)
    ('STOLARIK',                'Peter',         NULL),
    ('STYŚ',                    'Jan',           NULL),
    ('SULŻYC',                  'Piotr',         NULL),
    ('SZEPIETOWSKI',            'Rafał (kat 0)', NULL),
    ('SZKODA',                  'Marek Tomasz',  NULL),
    ('SZMAJDZIŃSKA',            '- BOŁDYS Katarzyna', 1992),
    ('SZMAJDZIŃSKA',            'Katarzyna',     NULL),
    ('SZMELC',                  '(0) Łukasz',    NULL),
    ('SZMELC',                  'Łukasz',        NULL),
    ('SZTYBER',                 'Michał',        NULL),
    ('SZYPUŁOWSKA-GRZYŚ',       'Joanna',        NULL),
    ('TATCHYN',                 'Andriy',        1980), -- ESTIMATED from V1 category (PPW4 Excel)
    ('TECŁAW',                  '(1) Robert',    NULL),
    ('UIJTING',                 'Henh',          NULL),
    ('VETULANI',                'Zygmunt',       NULL),
    ('WIERZBA',                 'Weronika',      NULL),
    ('WOJTAS',                  'Bogusław',      NULL),
    ('WOLAŃSKI',                'Adam',          NULL),
    ('WUJEK',                   'Dariusz',       NULL),
    ('ZAJĄC',                   '(1) Michał',    NULL),
    ('ZAKONEK',                 'Bronisław',     1950), -- ESTIMATED from V4 category (PPW4 Excel)
    ('ZAWROTNIAK',              'Radosław',      NULL),
    ('ZYLKA',                   'Henryk',        NULL),
    ('ŻYCZKOWSKI',              'KAROL',         NULL),
    ('TK',                      '',              1973);

-- Name aliases for identity resolution (M4/M5)
-- KOŃCZYŁO Tomasz competed as "TK" in some tournament systems
UPDATE tbl_fencer SET json_name_aliases = '["TK"]'
WHERE txt_surname = 'KOŃCZYŁO' AND txt_first_name = 'Tomasz';
