...
SELECT lives_ok(
  $$ SELECT fn_ingest_evf_calendar(
       '[{"name":"EVF Circuit – Guildford (GBR)","dt_start":"2039-01-09","dt_end":"2039-01-10","location":"Guildford Spectrum","country":"Great Britain","weapons":["EPEE"],"evf_calendar_id":2113,"evf_slug":"guildford-prior-link-regression","desired_code":"PEW1e-EVFPRIOR-CURR","existing_id_event":null}]'::JSONB,
       (SELECT id_season FROM tbl_season WHERE txt_code = 'EVFPRIOR-CURR'), 1
     ) $$,
...
SELECT is(
  (SELECT COUNT(*)::INT FROM tbl_event
    WHERE id_season = (SELECT id_season FROM tbl_season WHERE txt_code = 'EVFPRIOR-CURR')
      AND txt_code LIKE 'EVFLEGACY%'),
  1,
  '54.5: the wrong occupied slot is quarantined without deletion'
);
