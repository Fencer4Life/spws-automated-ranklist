-- =============================================================================
-- Registration switch is PROD-owned; venue address becomes fill-blank
-- =============================================================================
-- ADR-086 amendment / ADR-081.
--
-- 20260828000009 classified bool_use_spws_registration as "config that CERT
-- owns, so it overwrites whenever CERT states a value". That was an assumption,
-- and it was wrong. The column is NOT NULL DEFAULT false, so CERT ALWAYS states
-- a value: the COALESCE never falls through and the overwrite is unconditional.
--
-- With PPW1-2026-2027 sitting at CERT=false / PROD=true, the next promote would
-- have switched off a live public registration form holding 18 entries, silently
-- and unattended on the 06:00 schedule.
--
-- It is a PER-ENVIRONMENT OPERATIONAL SWITCH: registration is deliberately off
-- on CERT and on in PROD. A field whose whole purpose is to hold different
-- values in two environments cannot be synced. Removed from the UPDATE branch;
-- the CREATE branch still seeds a new event from CERT, and after that PROD owns
-- it outright -- the same treatment as ts_ftl_sent and the provenance block.
--
-- The general rule this settles, for every field the mirror carries:
--
--   SCRAPED FROM EVF     overwrite. txt_code, txt_name, dt_start, dt_end,
--                        txt_location, txt_country, arr_weapons, txt_evf_slug.
--                        EVF moves dates and renames events; freezing these
--                        leaves PROD stale and breaks the moved-date pill.
--   ADMIN ENRICHMENT     fill-blank. CERT seeds an empty PROD field; a value on
--                        PROD stops further propagation. The url_*, fee and
--                        contact fields -- and now txt_venue_address, which
--                        20260828000009 had made an overwrite.
--   PER-ENVIRONMENT      not synced after create. bool_use_spws_registration,
--                        ts_ftl_sent, the ingestion provenance block, and the
--                        results half of enum_status.
--
-- Reproduced from the LIVE pg_get_functiondef output so 20260828000010-13 carry
-- forward.
--
-- Plan-test-ID 68 (supabase/tests/68_prod_mirror_field_ownership.sql).
-- =============================================================================
