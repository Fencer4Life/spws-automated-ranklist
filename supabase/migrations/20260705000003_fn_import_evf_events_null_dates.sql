-- =============================================================================
-- fn_import_evf_events: accept empty-string dt_start/dt_end as NULL
-- =============================================================================
-- The CERT->PROD calendar promoter (python/pipeline/promote.py::_build_import_payload)
-- always emits "" rather than JSON null for missing date fields, to keep the
-- payload shape stable across all keys. dt_registration_deadline already guards
-- against this with NULLIF(...,'')::DATE, but dt_start/dt_end cast the raw
-- JSONB text directly, so any EVF event with a NULL dt_start on CERT (both
-- columns are nullable on tbl_event) makes the whole promote run fail with
-- "invalid input syntax for type date: ''" (2026-07-02..07-05 EVF Calendar +
-- Results Sync workflow failures). Apply the same NULLIF idiom here.
-- =============================================================================
...
BEGIN
  ...
  LOOP
    DECLARE
      ...
      v_dt_start       DATE := NULLIF(v_evt ->> 'dt_start', '')::DATE;
      v_dt_end         DATE := COALESCE(NULLIF(v_evt ->> 'dt_end', '')::DATE, v_dt_start);
