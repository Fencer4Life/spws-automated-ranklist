-- =============================================================================
-- ADR-044 (amend) / ADR-017 follow-up: split the +EVF toggle into two flags.
--
-- SPWS lost its national-team-appointment privilege, so the Ranklist +EVF/Kadra
-- toggle must default OFF (bool_show_evf_toggle already defaults FALSE — kept as
-- the RANKLIST flag). The Calendar, however, should still show the richer EVF +
-- FIE view by default, so this adds a SECOND, independent flag:
--   bool_show_evf_toggle_calendar BOOLEAN NOT NULL DEFAULT TRUE
--
-- Extends fn_export_scoring_config (JSONB out) and fn_import_scoring_config
-- (JSONB in) to carry show_evf_toggle_calendar, mirroring the existing
-- show_evf_toggle lines from 20260329000002_evf_toggle_config.sql.
-- =============================================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'tbl_scoring_config' AND column_name = 'bool_show_evf_toggle_calendar'
  ) THEN
    ALTER TABLE tbl_scoring_config
      ADD COLUMN bool_show_evf_toggle_calendar BOOLEAN NOT NULL DEFAULT TRUE;
  END IF;
END
$$;
...
-- -----------------------------------------------------------------------------
-- fn_export_scoring_config — add show_evf_toggle_calendar to JSONB output
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- fn_import_scoring_config — accept show_evf_toggle_calendar in JSONB input
-- -----------------------------------------------------------------------------
