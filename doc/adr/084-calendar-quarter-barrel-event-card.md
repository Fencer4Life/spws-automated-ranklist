...
**§2's `translateY` rotation is superseded.** Rows are placed on the surface of a cylinder — `rotateX(i·θ) translateZ(R)`, with `R = (rowHeight/2) / tan(θ/2)`. Foreshortening at the rim, the compression of spacing toward the horizon, occlusion and the horizon itself fall out of the perspective projection instead of being drawn. At θ = 26° the drum carries **seven rows**, with `ROW_H = 84px` giving `R = 182px`. Time runs **upward**: the future sits above the focused row, the past below, inverting the flat drum's `.ln.up = active − 1`.

**The drum does not rotate as one body, and must not.** Rotating the container by `-active · θ` is the obvious reading of a cylinder and it fails silently: by row 63 that is `rotateX(-1638deg)`, and the browser **drops a rotation of that magnitude outright** — the computed drum matrix comes back as a bare `translateZ(-182px)` while each row keeps its own large rotation. The residual is not a wrong angle, it is a *growing* one: the focused row rendered 55px instead of 84 with 49° of unwanted tilt, worsening the deeper the drum was rolled, which reads as the focused tiles sliding into the seam below. Each row therefore carries its own **relative** angle, `rotateX((i − active) · θ)`, which is bounded by the visible span and never approaches the limit; the container only holds `translateZ(-R)`. `CB.3c` pins the bound.

**Receded rows are scaled, not merely foreshortened.** Perspective alone left them nearly as tall as the focused row, so the drum spent its height on context instead of content. `RECEDED_SCALE = 0.62` shrinks every non-focused row, buying the focused row the vertical room its enlarged type needs.

**Seams run coast to coast.** Each seam reaches both viewport edges, inset only enough to give its label air. Because a receded row is scaled by `RECEDED_SCALE`, a seam declared at `100%` would render at 62% of the viewport, so the declared width divides by the row's own scale and the padding does likewise — otherwise the label inset shrinks on exactly the rows with least room. This **supersedes** the earlier rule that capped the focused seam to `RECEDED_SCALE` to make all seams *equally narrow*: at 320px that left too little width to render the jump control, which is what forced the question. The residual difference between rows is perspective — a row tilted 26° sits ~18px further from the eye and renders ~2.4% narrower — and is under a pixel at this size.

**The focused row's tiles are centred between the two seams.** They start unequal for a structural reason: the focused row's own seam is 14px tall with its rule centred, so ~7px of seam sits between that rule and the tiles, while the row below is scaled to 62%, giving a 7px seam whose rule sits ~3px from its top. Measured to the hairlines — which is what the eye reads — that is 10px above against 5px below. The correction is **half** the difference, because moving the tiles up by *x* closes the gap above by *x* and opens the one below by the same *x*; 2.5px puts both at 7.5px. It is applied as row-internal spacing, never as an offset on the row's own box: shifting `.ln.mid` moves its transform-origin off the cylinder axis, and moves the row's seam *with* the tiles, so it cannot change the gap above at all.
...
**A jump control resolves the drum-depth question** left open above. It appears **only when the focused row is not the opening row**, so it costs nothing in the common case and its appearance is itself the signal that you have drifted. It pins to the **viewport edge** the drum must travel towards — the top when the anchor is ahead (`dn`, a later month, which renders above), the bottom when it is behind — so its position is the direction cue and no arrow is needed.
...
The jump is **instant**. Crossing forty rows is over a thousand degrees, nearly three full turns, the same failure the opening frame avoids.

**It is a sibling of the drum, not a child of the row it points at**, and that placement is load-bearing. Parented to a receded row it inherited `RECEDED_SCALE`, so it had to scale by the inverse to stay legible, and a ~32px pill grown to ~52px inside a ~7px seam covered that row's tiles outright — unusable at 320px. Out at the viewport edge it needs no scale compensation and can collide with nothing but the extreme rows, which are near edge-on and faded to almost nothing. It also no longer needs `stopPropagation`: it is not inside a tap target any more, so the jump and a single step can no longer fight over one tap. `CB.29` keeps that guarantee under test, because re-nesting the control would restore the conflict.
