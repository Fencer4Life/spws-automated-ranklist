...
2. **New `FencerView.svelte`** tab container wrapping two tabs:
...
5. **Birth year review** features:
   ...
   - Click to expand edit form with read-only fencer info + editable birth year/accuracy
...
- IdentityManager.svelte preserved with zero changes — re-parented under FencerView
- New components: `FencerView.svelte`, `BirthYearReview.svelte`
- New RPC: `fn_update_fencer_birth_year` (migration `20260412000004`)
- UC16 partially implemented (birth year editing done; club/nationality editing and fencer merge deferred)
- Test count: pgTAP 254→259 (+5), vitest 241→260 (+19)
