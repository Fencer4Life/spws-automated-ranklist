...
| **Logical-integrity self-heal (ADR-039 Step 0 rev 2)** | `<b>EVF Sync self-healed {n} row(s)</b>\n<pre>{codes}</pre>\nDates re-derived from FTL.` | `sync_calendar` rewrote a future-COMPLETED row's date from the authoritative FTL results page. Informational — the sync continues. No action needed. |
| **Logical-integrity violation, un-healable (ADR-039 Step 0 rev 2)** | `<b>EVF Sync HALT</b>\n<pre>Future-COMPLETED, un-healable: {code} dt_start={date}</pre>\nManual fix required.` | Self-heal could not recover a valid past date from FTL (results pulled, or FTL date still future). Fix the row's `dt_start` manually in CERT, then re-run. The GH Actions `Notify failure` step also fires the generic FAILED message. |
| **Self-heal blocked by FTL auth (ADR-039 Step 0 rev 2)** | `<b>EVF Sync self-heal blocked</b>\n<pre>{FtlAuthError}</pre>` | The authed FTL session could not be established (missing/rotated `FTL_USERNAME`/`FTL_PASSWORD` secrets). Update the GitHub secrets; until then every future-COMPLETED row is un-healable. |
