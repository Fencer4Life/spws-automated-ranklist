...
| 3 — Stages 1-7 + alias writeback + 3-way diff + interactive CLI | ✅ DONE | 2026-05-02 | Migration `20260502000001_phase3_fn_alias_writeback.sql` (8 pgTAP — `28.1`–`28.8`): `fn_update_fencer_aliases` with NULL init, case-insensitive dedup, whitespace trim, empty-reject. New modules: `python/pipeline/types.py` (HaltError + Overrides + PipelineContext + StageMatchResult; 8 pytest), `python/pipeline/overrides.py` (5-surface YAML parser; 15 pytest), `python/pipeline/stages.py` (S1-S7 free functions, halt-by-exception; 29 pytest), `python/pipeline/three_way_diff.py` (4-bucket classifier + histogram + markdown; 12 pytest), `python/pipeline/review_cli.py` (ReviewSession with 4 source-of-truth choices incl. EVF API; 15 pytest). Updated `python/pipeline/orchestrator.py` (added `run_pipeline` dispatcher; legacy `process_xml_file` annotated with deprecation note). Updated `python/pipeline/db_connector.py` (added `find_event_by_code`, `fetch_cert_rows_for_event` stub, `call_age_categories_batch`). 4 end-to-end integration tests (`test_pipeline_integration.py`) skip cleanly when LOCAL DB unreachable. 5 design decisions Q1-Q5 locked in micro-RFC, captured in ADR-050 §"Status — Phase 3 deliverables". 4 deferrals to Phase 4: frozen-snapshot, production fetcher wiring, real cert_ref query, EVF parity gate, URL→data deep validation. |
| 4 — Commit path + EVF parity + alias UI | ✅ DONE | 2026-05-02 | Migrations `20260502000002_phase4_evf_parity_lifecycle.sql` (12 pgTAP), `20260502000003_phase4_alias_management.sql` (16 pgTAP), `20260502000004` cert_ref enum + `…000005` cert_ref query RPCs, `…000006_pew_cascade_hook.sql` (5 pgTAP), `…000007_evf_promote_rpcs.sql` (10 pgTAP) — `fn_promote_evf_published`/`fn_annotate_parity_fail`/`fn_event_results_for_parity`/`fn_evf_events_pending_parity`. Python: `url_validation.py` (Stage 7 6-field halt + PEW cascade flag, 20 pytest), `evf_parity.py` (3 sub-checks ±0.5, 12 pytest), `commit_lifecycle.py` (post-commit cascade + parity + batched Telegram, 7 pytest), `evf_parity_sweep.py` (daily cron with 30d retry window, 5 pytest), `cert_ref.py` parser (8 pytest). `db_connector.py` extended with parity + cascade RPC wrappers. `notifications.py` extended with 7 Phase-4 templates (commit / promote / parity-fail / halt / cron summary / cascade / EVF empty; 8 pytest). `review_cli.py` wired post-commit hooks + Telegram halt notification. New mockup `doc/mockups/m11_fencer_aliases.html` with 4 design options (Expandable / Cards / Two-pane / Dense). New Svelte component `FencerAliasManager.svelte` (6 vitest) + `frontend/src/lib/api.ts` wrappers `listFencerAliases`/`transferFencerAlias`/`splitFencerFromAlias`/`discardFencerAliasAndResults` (4 vitest). `FencerWithAliases` type added. New GitHub Actions workflow `.github/workflows/evf-parity-sweep.yml` (daily 04:00 UTC, prod-write concurrency group). |
