...
| FR-97 | EVF logical-integrity guard: a `tbl_event` row with `dt_start > today AND enum_status = 'COMPLETED'` is data corruption. **ADR-039 rev 2 (2026-06-13):** before halting, `sync_calendar` self-heals via `_heal_future_completed` — re-deriving the real date from the authoritative FTL results page (authed FTL session, ADR-069) and rewriting `dt_start`/`dt_end` + `dt_tournament` on CERT (`compute_future_completed_corrections` decides; pure). Only un-healable rows (no FTL date, `FtlAuthError`, or FTL date still future) keep the rev-1 hard halt: `assert_no_future_completed` raises `LogicalIntegrityError`, the **EVF Sync HALT** Telegram alert fires, exit non-zero for manual reconciliation. | UC25, UC27, ADR-039, ADR-069 | pytest evf.23, evf.44–evf.48 | Covered |
