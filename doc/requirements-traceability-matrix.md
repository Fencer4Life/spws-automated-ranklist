...
|-----|-------|-------|--------|
| [ADR-050](adr/050-unified-ingestion-pipeline.md) | Unified Ingestion Pipeline — IR contract + 8 parsers + parser registry (Phase 1); draft tables + dry-run loop + commit/discard RPCs (Phase 2) | **Phase 1**: `python/tests/test_ir.py::ir.1`–`ir.7`; `python/tests/test_ir_contracts.py` (41 parser-contract assertions across 9 test classes). **Phase 2**: `supabase/tests/27_draft_tables.sql::27.1`–`27.26` (30 pgTAP — table shape, FK absence, RPC signatures, fn_commit_event_draft move semantics + joint-pool flag + audit + history + idempotency, fn_discard_event_draft, fn_dry_run_event_draft no-persist); `python/tests/test_draft_store.py::P2.D1`–`P2.D9` (9 DraftStore unit tests); `python/tests/test_draft_diff.py::P2.M1`–`P2.M5` (5 markdown formatter tests); `python/tests/test_ingest_cli.py::P2.C1`–`P2.C6` (6 CLI flag tests, incl. zero-count Telegram routing + mutex) | Covered (Phase 1 2026-05-01; Phase 2 2026-05-02) |
