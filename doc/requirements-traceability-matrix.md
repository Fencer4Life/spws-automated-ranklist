...
| FR-71 | Orchestrator splits combined-category XML (v0v1, v0v1v2) into per-category result sets with re-ranking; structural pool-only skip drops ELIMINACJE qualifier files (`<Poule>` without `<Tableau>`) at the parser so they never reach the splitter (ADR-067) | ADR-024, ADR-067 | 9.152–9.154, 67.P1–67.P8 (pool-only structural skip — parser flag, pipeline halt, legacy path skip, Sexe='X'-but-has-DE regression guard, live PPW5 file sanity) | Covered |
