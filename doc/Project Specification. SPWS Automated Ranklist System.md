...
| [ADR-039](adr/039-stale-event-gate.md) | EVF Scraper Dedup Algorithm + Stale-Event Gate. **Rev 7 (2026-08-09):** the former empty-duplicate assumption is superseded by scored fragmentation incidents. Automated ingest still fails closed; there is no generic merge RPC. A scored conflict requires an exact-baseline, source-verified, event-specific migration that refuses registrations, conserves match-candidate provenance through unchanged result identities, and deletes only emptied named donors. | FR-58, FR-96, FR-97, ADR-028, ADR-069 |
