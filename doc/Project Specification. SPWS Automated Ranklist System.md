...
| [ADR-039](adr/039-stale-event-gate.md) | EVF Scraper Dedup Algorithm + Stale-Event Gate (amends ADR-028; rev 2 2026-06-13: Step 0 self-heals future-COMPLETED rows — recover date from the event URL, else demote COMPLETED→PLANNED — so the sync never hard-halts on this class) | FR-58, FR-96, FR-97, ADR-028, ADR-069 |
...
| [ADR-046](adr/046-pew-weapon-suffix.md) | PEW event codes carry weapon-letter suffix (`PEW{N}{efs}-{season}`); one event = one physical weekend; splitter migration unbundles bundled events; amended 2026-06-25 — splitter is collision-resilient: prunes empty placeholder child slots not matching the event's code suffix, and resolves weapon-derived code collisions by provenance (merge empty duplicate / skip+WARNING on result-bearing conflict) instead of aborting the seed load on `idx_event_code`; never deletes a result (test 47) | ADR-043, ADR-044 |
