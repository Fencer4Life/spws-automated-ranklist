...
### 5.2 Event Registration & Clean-Roster Seeding (in progress — Phases 1–2 done, Phase 3 partial, Phases 4–5 not started)
...
Design detail: ADR-078 (GDPR), ADR-079 (registration/identity, read-only invariant + Model B identity + impersonation bounding), ADR-080 (seeding). New `tbl_event` fields: `url_entry_list`, `txt_organizer_email`, `ts_ftl_sent`, `num_entry_fee_2w`, `num_entry_fee_3w`, `bool_use_spws_registration`. New table `tbl_registration` (ephemeral, RLS-gated). UI mockups: `doc/mockups/registration_*.html`. FRs: FR-120–FR-130 (RTM).

**Status (2026-07-05).** Phase 1 (DB schema) and Phase 2 (public registration UI) are **done**: the standalone shareable page `frontend/register.html` renders `<spws-registration>`/`<spws-entry-list>` (identity → exact-match routing → RODO consent → payment; D8 mobile-first, D10 expiry-gated both client- and server-side) and the admin `EventManager`/`CalendarView` integration (FR-130). Phase 3 (FTL seed export) is **partial**: pure logic + mix-all DB orchestration are done; per-bracket DE seed files are deferred (no validated reference format). Phases 4 (`send_seed_to_organizer` email delivery) and 5 (ingestion wiring) are **not started** — blocked on Resend/eu.org email infra.
