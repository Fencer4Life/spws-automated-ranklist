...
**In scope.** Domestic PPW/MPW/GP self-registration; a bilingual (PL/EN) public form + public entry list; generation of FTL FIE-XML seed files from paid registrations; on-demand delivery to the organizer.
**Out of scope.** International events (PEW/MEW/MSW); a card-payment gateway (payment is display-only bank transfer); automated migration off competit.pl; any write to `tbl_fencer` / results / ranking from the form.

**Actors.** Fencer (self-registers), Admin (configures the event, triggers/oversees delivery, reviews quarantined conflicts), Organizer (receives the seed, runs FTL), System (deadline cron, Telegram command).

**Definitions.**
- **Registration** — one fencer's declared entry to one event: identity + weapon/category selections + payment state, held in the ephemeral `tbl_registration`.
- **Paid** — a registration whose entry fee has been reconciled (`enum_payment_status = PAID`); only paid registrations are authoritative.
...
- Only **paid** registrations enter the seed and the authoritative entry list.
...
| [ADR-079](adr/079-event-self-registration-identity.md) | **Draft.** Event self-registration & identity resolution — **read-only invariant on `tbl_fencer`** (form writes only ephemeral `tbl_registration`; reconciliation runs at ingestion, declared BY replaces estimation); Model B tuple gate (anonymous on exact Surname+Name+BY, else one-time email = friction not identity, no account); **confirmed-BY-sacrosanct** matrix; impersonation hardening via payment-gated roster + results-based ranking; `tbl_registration` as rows not a JSON file (concurrency/RLS/GDPR). New `tbl_event` fields + entry-list. | spec §5.2, ADR-078, ADR-080, ADR-056, ADR-030 |
| [ADR-080](adr/080-clean-roster-ftl-seeding.md) | **Draft.** Clean-roster FTL (Fencing Time) seeding — FIE XML `<BaseCompetitionIndividuelle>` per competition; **omit `DateNaissance`** (FTL infers category from it); per-fencer `(N)` category marker in `Prenom` (gender via structural `Sexe`, K/M dropped); mix-all pool interleave seeding across the 10 sub-rankings; predicted combined DE brackets (T=4, adjacent ascending, genders never merged); naming `<season>_<code>_<weapon>_<scope>`; on-demand organizer-email delivery (`txt_organizer_email`, generate-at-send from paid regs) via 3 triggers (EventManager button / deadline cron / Telegram `/seed`). Round-trip validated FTL v4.5.4. Interleave seeding fully specified (exact round-robin-by-rank algorithm + worked EPEE example + preserved example XML artefacts in `doc/external_files/FTL_SRC/`). | spec §5.2, ADR-078, ADR-079, ADR-024, ADR-065 |
