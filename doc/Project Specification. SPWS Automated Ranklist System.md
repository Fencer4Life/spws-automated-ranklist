...
| [ADR-079](adr/079-event-self-registration-identity.md) | **Draft.** Event self-registration & identity resolution — **read-only invariant on `tbl_fencer`** (form writes only ephemeral `tbl_registration`; reconciliation runs at ingestion, declared BY replaces estimation); Model B tuple gate (anonymous on exact Surname+Name+BY, else one-time email = friction not identity, no account); **confirmed-BY-sacrosanct** matrix; impersonation hardening via **venue-level in-person payment check** (not a software payment gate — corrected 2026-07-04, this system does not track payment completion) + results-based ranking; `tbl_registration` as rows not a JSON file (concurrency/RLS/GDPR). New `tbl_event` fields + entry-list. | spec §5.2, ADR-078, ADR-080, ADR-056, ADR-030 |
