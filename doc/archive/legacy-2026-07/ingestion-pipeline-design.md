...
> **ARCHIVED.** This document records the transitional stage-layer design. Use the current [ingestion subsystem](../../handbook/subsystems/ingestion.html) for present behavior.
...
**Status:** Historical stage-layer reference · **Scope:** the end-to-end ingestion pipeline as documented at the time
...
| RPC | Remote Procedure Call | A callable PL/pgSQL function exposed to the app (e.g. `fn_commit_event_draft`) | [§6](#6-code-chunk-reference-table-file-paths) |
...
| CLI | Command-Line Interface | `ingest_cli.py` / `review_cli.py` entry points | [§6](#6-code-chunk-reference-table-file-paths) |
...
Read top-to-bottom. Each node is a real module/class/function (file path in [§6](#6-code-chunk-reference-table-file-paths)).
